...
// https://support.socrata.com/hc/en-us/articles/4403481607447-Date-and-Time-Transformation-Overview
...
export default function TripChart ({ data, loading, error }) {
  ...
  console.log("trips are aggregated to UTC day - there's no way around this other than pulling all the damn data - maybe a socrata function in the dataset field?")

  if (!showDaily) {
    data = data.reduce(reduceToMonth, []);
  } else {
    data.forEach((trip) => {
      trip.date = new Date(trip.date + "z").toLocaleDateString();
